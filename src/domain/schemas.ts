import { z } from "zod";
import { cognitiveFunctions, epistemicStatuses, eventTypes, speechActs, thoughtEdgeTypes, thoughtNodeTypes, thoughtPhases, userMoveKinds } from "./types";
...
export const providerInputSchema = providerBase.superRefine((value, context) => {
  if (value.kind !== "mock" && !value.modelId?.trim()) context.addIssue({ code: "custom", path: ["modelId"], message: "真实供应商必须填写 Model ID" });
...
export const functionModelInputSchema = z.object({ cognitiveFunction: cognitiveFunctionSchema, providerId: z.string().nullable(), modelId: z.string().trim().max(200).nullable() });
...
export const sessionBundleSchema = z.object({
  ...
  nodes: z.array(z.object({ id: z.string(), sessionId: z.string(), type: thoughtNodeTypeSchema, content: z.string(), author: z.enum(["user", "system"]), epistemicStatus: epistemicStatusSchema, parentNodeId: z.string().nullable(), sourceEventIds: z.array(z.string()), speechAct: speechActSchema.nullable(), confirmable: z.boolean(), provenanceNodeId: z.string().nullable(), createdAt: z.number(), updatedAt: z.number() })),
  ...
  runs: z.array(z.object({ id: z.string(), sessionId: z.string(), eventId: z.string().nullable(), providerId: z.string(), modelId: z.string().nullable(), mode: z.enum(["mock", "real"]), status: z.enum(["running", "completed", "failed", "aborted"]), errorMessage: z.string().nullable(), startedAt: z.number(), completedAt: z.number().nullable() })).optional(),
});
